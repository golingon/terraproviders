// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package vault_ldap_secret_backend

import (
	"encoding/json"
	"fmt"
	"github.com/golingon/lingon/pkg/terra"
	"io"
)

// New creates a new instance of [Resource].
func New(name string, args Args) *Resource {
	return &Resource{
		Args: args,
		Name: name,
	}
}

var _ terra.Resource = (*Resource)(nil)

// Resource represents the Terraform resource vault_ldap_secret_backend.
type Resource struct {
	Name      string
	Args      Args
	state     *vaultLdapSecretBackendState
	DependsOn terra.Dependencies
	Lifecycle *terra.Lifecycle
}

// Type returns the Terraform object type for [Resource].
func (vlsb *Resource) Type() string {
	return "vault_ldap_secret_backend"
}

// LocalName returns the local name for [Resource].
func (vlsb *Resource) LocalName() string {
	return vlsb.Name
}

// Configuration returns the configuration (args) for [Resource].
func (vlsb *Resource) Configuration() interface{} {
	return vlsb.Args
}

// DependOn is used for other resources to depend on [Resource].
func (vlsb *Resource) DependOn() terra.Reference {
	return terra.ReferenceResource(vlsb)
}

// Dependencies returns the list of resources [Resource] depends_on.
func (vlsb *Resource) Dependencies() terra.Dependencies {
	return vlsb.DependsOn
}

// LifecycleManagement returns the lifecycle block for [Resource].
func (vlsb *Resource) LifecycleManagement() *terra.Lifecycle {
	return vlsb.Lifecycle
}

// Attributes returns the attributes for [Resource].
func (vlsb *Resource) Attributes() vaultLdapSecretBackendAttributes {
	return vaultLdapSecretBackendAttributes{ref: terra.ReferenceResource(vlsb)}
}

// ImportState imports the given attribute values into [Resource]'s state.
func (vlsb *Resource) ImportState(state io.Reader) error {
	vlsb.state = &vaultLdapSecretBackendState{}
	if err := json.NewDecoder(state).Decode(vlsb.state); err != nil {
		return fmt.Errorf("decoding state into resource %s.%s: %w", vlsb.Type(), vlsb.LocalName(), err)
	}
	return nil
}

// State returns the state and a bool indicating if [Resource] has state.
func (vlsb *Resource) State() (*vaultLdapSecretBackendState, bool) {
	return vlsb.state, vlsb.state != nil
}

// StateMust returns the state for [Resource]. Panics if the state is nil.
func (vlsb *Resource) StateMust() *vaultLdapSecretBackendState {
	if vlsb.state == nil {
		panic(fmt.Sprintf("state is nil for resource %s.%s", vlsb.Type(), vlsb.LocalName()))
	}
	return vlsb.state
}

// Args contains the configurations for vault_ldap_secret_backend.
type Args struct {
	// AllowedManagedKeys: set of string, optional
	AllowedManagedKeys terra.SetValue[terra.StringValue] `hcl:"allowed_managed_keys,attr"`
	// AuditNonHmacRequestKeys: list of string, optional
	AuditNonHmacRequestKeys terra.ListValue[terra.StringValue] `hcl:"audit_non_hmac_request_keys,attr"`
	// AuditNonHmacResponseKeys: list of string, optional
	AuditNonHmacResponseKeys terra.ListValue[terra.StringValue] `hcl:"audit_non_hmac_response_keys,attr"`
	// Binddn: string, required
	Binddn terra.StringValue `hcl:"binddn,attr" validate:"required"`
	// Bindpass: string, required
	Bindpass terra.StringValue `hcl:"bindpass,attr" validate:"required"`
	// Certificate: string, optional
	Certificate terra.StringValue `hcl:"certificate,attr"`
	// ClientTlsCert: string, optional
	ClientTlsCert terra.StringValue `hcl:"client_tls_cert,attr"`
	// ClientTlsKey: string, optional
	ClientTlsKey terra.StringValue `hcl:"client_tls_key,attr"`
	// ConnectionTimeout: number, optional
	ConnectionTimeout terra.NumberValue `hcl:"connection_timeout,attr"`
	// DefaultLeaseTtlSeconds: number, optional
	DefaultLeaseTtlSeconds terra.NumberValue `hcl:"default_lease_ttl_seconds,attr"`
	// Description: string, optional
	Description terra.StringValue `hcl:"description,attr"`
	// DisableRemount: bool, optional
	DisableRemount terra.BoolValue `hcl:"disable_remount,attr"`
	// ExternalEntropyAccess: bool, optional
	ExternalEntropyAccess terra.BoolValue `hcl:"external_entropy_access,attr"`
	// Id: string, optional
	Id terra.StringValue `hcl:"id,attr"`
	// InsecureTls: bool, optional
	InsecureTls terra.BoolValue `hcl:"insecure_tls,attr"`
	// Local: bool, optional
	Local terra.BoolValue `hcl:"local,attr"`
	// MaxLeaseTtlSeconds: number, optional
	MaxLeaseTtlSeconds terra.NumberValue `hcl:"max_lease_ttl_seconds,attr"`
	// Namespace: string, optional
	Namespace terra.StringValue `hcl:"namespace,attr"`
	// Options: map of string, optional
	Options terra.MapValue[terra.StringValue] `hcl:"options,attr"`
	// PasswordPolicy: string, optional
	PasswordPolicy terra.StringValue `hcl:"password_policy,attr"`
	// Path: string, optional
	Path terra.StringValue `hcl:"path,attr"`
	// RequestTimeout: number, optional
	RequestTimeout terra.NumberValue `hcl:"request_timeout,attr"`
	// Schema: string, optional
	Schema terra.StringValue `hcl:"schema,attr"`
	// SealWrap: bool, optional
	SealWrap terra.BoolValue `hcl:"seal_wrap,attr"`
	// SkipStaticRoleImportRotation: bool, optional
	SkipStaticRoleImportRotation terra.BoolValue `hcl:"skip_static_role_import_rotation,attr"`
	// Starttls: bool, optional
	Starttls terra.BoolValue `hcl:"starttls,attr"`
	// Upndomain: string, optional
	Upndomain terra.StringValue `hcl:"upndomain,attr"`
	// Url: string, optional
	Url terra.StringValue `hcl:"url,attr"`
	// Userattr: string, optional
	Userattr terra.StringValue `hcl:"userattr,attr"`
	// Userdn: string, optional
	Userdn terra.StringValue `hcl:"userdn,attr"`
}

type vaultLdapSecretBackendAttributes struct {
	ref terra.Reference
}

// Accessor returns a reference to field accessor of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Accessor() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("accessor"))
}

// AllowedManagedKeys returns a reference to field allowed_managed_keys of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) AllowedManagedKeys() terra.SetValue[terra.StringValue] {
	return terra.ReferenceAsSet[terra.StringValue](vlsb.ref.Append("allowed_managed_keys"))
}

// AuditNonHmacRequestKeys returns a reference to field audit_non_hmac_request_keys of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) AuditNonHmacRequestKeys() terra.ListValue[terra.StringValue] {
	return terra.ReferenceAsList[terra.StringValue](vlsb.ref.Append("audit_non_hmac_request_keys"))
}

// AuditNonHmacResponseKeys returns a reference to field audit_non_hmac_response_keys of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) AuditNonHmacResponseKeys() terra.ListValue[terra.StringValue] {
	return terra.ReferenceAsList[terra.StringValue](vlsb.ref.Append("audit_non_hmac_response_keys"))
}

// Binddn returns a reference to field binddn of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Binddn() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("binddn"))
}

// Bindpass returns a reference to field bindpass of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Bindpass() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("bindpass"))
}

// Certificate returns a reference to field certificate of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Certificate() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("certificate"))
}

// ClientTlsCert returns a reference to field client_tls_cert of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) ClientTlsCert() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("client_tls_cert"))
}

// ClientTlsKey returns a reference to field client_tls_key of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) ClientTlsKey() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("client_tls_key"))
}

// ConnectionTimeout returns a reference to field connection_timeout of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) ConnectionTimeout() terra.NumberValue {
	return terra.ReferenceAsNumber(vlsb.ref.Append("connection_timeout"))
}

// DefaultLeaseTtlSeconds returns a reference to field default_lease_ttl_seconds of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) DefaultLeaseTtlSeconds() terra.NumberValue {
	return terra.ReferenceAsNumber(vlsb.ref.Append("default_lease_ttl_seconds"))
}

// Description returns a reference to field description of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Description() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("description"))
}

// DisableRemount returns a reference to field disable_remount of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) DisableRemount() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("disable_remount"))
}

// ExternalEntropyAccess returns a reference to field external_entropy_access of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) ExternalEntropyAccess() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("external_entropy_access"))
}

// Id returns a reference to field id of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Id() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("id"))
}

// InsecureTls returns a reference to field insecure_tls of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) InsecureTls() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("insecure_tls"))
}

// Local returns a reference to field local of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Local() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("local"))
}

// MaxLeaseTtlSeconds returns a reference to field max_lease_ttl_seconds of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) MaxLeaseTtlSeconds() terra.NumberValue {
	return terra.ReferenceAsNumber(vlsb.ref.Append("max_lease_ttl_seconds"))
}

// Namespace returns a reference to field namespace of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Namespace() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("namespace"))
}

// Options returns a reference to field options of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Options() terra.MapValue[terra.StringValue] {
	return terra.ReferenceAsMap[terra.StringValue](vlsb.ref.Append("options"))
}

// PasswordPolicy returns a reference to field password_policy of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) PasswordPolicy() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("password_policy"))
}

// Path returns a reference to field path of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Path() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("path"))
}

// RequestTimeout returns a reference to field request_timeout of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) RequestTimeout() terra.NumberValue {
	return terra.ReferenceAsNumber(vlsb.ref.Append("request_timeout"))
}

// Schema returns a reference to field schema of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Schema() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("schema"))
}

// SealWrap returns a reference to field seal_wrap of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) SealWrap() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("seal_wrap"))
}

// SkipStaticRoleImportRotation returns a reference to field skip_static_role_import_rotation of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) SkipStaticRoleImportRotation() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("skip_static_role_import_rotation"))
}

// Starttls returns a reference to field starttls of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Starttls() terra.BoolValue {
	return terra.ReferenceAsBool(vlsb.ref.Append("starttls"))
}

// Upndomain returns a reference to field upndomain of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Upndomain() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("upndomain"))
}

// Url returns a reference to field url of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Url() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("url"))
}

// Userattr returns a reference to field userattr of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Userattr() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("userattr"))
}

// Userdn returns a reference to field userdn of vault_ldap_secret_backend.
func (vlsb vaultLdapSecretBackendAttributes) Userdn() terra.StringValue {
	return terra.ReferenceAsString(vlsb.ref.Append("userdn"))
}

type vaultLdapSecretBackendState struct {
	Accessor                     string            `json:"accessor"`
	AllowedManagedKeys           []string          `json:"allowed_managed_keys"`
	AuditNonHmacRequestKeys      []string          `json:"audit_non_hmac_request_keys"`
	AuditNonHmacResponseKeys     []string          `json:"audit_non_hmac_response_keys"`
	Binddn                       string            `json:"binddn"`
	Bindpass                     string            `json:"bindpass"`
	Certificate                  string            `json:"certificate"`
	ClientTlsCert                string            `json:"client_tls_cert"`
	ClientTlsKey                 string            `json:"client_tls_key"`
	ConnectionTimeout            float64           `json:"connection_timeout"`
	DefaultLeaseTtlSeconds       float64           `json:"default_lease_ttl_seconds"`
	Description                  string            `json:"description"`
	DisableRemount               bool              `json:"disable_remount"`
	ExternalEntropyAccess        bool              `json:"external_entropy_access"`
	Id                           string            `json:"id"`
	InsecureTls                  bool              `json:"insecure_tls"`
	Local                        bool              `json:"local"`
	MaxLeaseTtlSeconds           float64           `json:"max_lease_ttl_seconds"`
	Namespace                    string            `json:"namespace"`
	Options                      map[string]string `json:"options"`
	PasswordPolicy               string            `json:"password_policy"`
	Path                         string            `json:"path"`
	RequestTimeout               float64           `json:"request_timeout"`
	Schema                       string            `json:"schema"`
	SealWrap                     bool              `json:"seal_wrap"`
	SkipStaticRoleImportRotation bool              `json:"skip_static_role_import_rotation"`
	Starttls                     bool              `json:"starttls"`
	Upndomain                    string            `json:"upndomain"`
	Url                          string            `json:"url"`
	Userattr                     string            `json:"userattr"`
	Userdn                       string            `json:"userdn"`
}
