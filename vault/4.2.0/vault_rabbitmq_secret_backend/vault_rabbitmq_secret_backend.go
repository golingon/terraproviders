// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package vault_rabbitmq_secret_backend

import (
	"encoding/json"
	"fmt"
	"github.com/golingon/lingon/pkg/terra"
	"io"
)

// New creates a new instance of [Resource].
func New(name string, args Args) *Resource {
	return &Resource{
		Args: args,
		Name: name,
	}
}

var _ terra.Resource = (*Resource)(nil)

// Resource represents the Terraform resource vault_rabbitmq_secret_backend.
type Resource struct {
	Name      string
	Args      Args
	state     *vaultRabbitmqSecretBackendState
	DependsOn terra.Dependencies
	Lifecycle *terra.Lifecycle
}

// Type returns the Terraform object type for [Resource].
func (vrsb *Resource) Type() string {
	return "vault_rabbitmq_secret_backend"
}

// LocalName returns the local name for [Resource].
func (vrsb *Resource) LocalName() string {
	return vrsb.Name
}

// Configuration returns the configuration (args) for [Resource].
func (vrsb *Resource) Configuration() interface{} {
	return vrsb.Args
}

// DependOn is used for other resources to depend on [Resource].
func (vrsb *Resource) DependOn() terra.Reference {
	return terra.ReferenceResource(vrsb)
}

// Dependencies returns the list of resources [Resource] depends_on.
func (vrsb *Resource) Dependencies() terra.Dependencies {
	return vrsb.DependsOn
}

// LifecycleManagement returns the lifecycle block for [Resource].
func (vrsb *Resource) LifecycleManagement() *terra.Lifecycle {
	return vrsb.Lifecycle
}

// Attributes returns the attributes for [Resource].
func (vrsb *Resource) Attributes() vaultRabbitmqSecretBackendAttributes {
	return vaultRabbitmqSecretBackendAttributes{ref: terra.ReferenceResource(vrsb)}
}

// ImportState imports the given attribute values into [Resource]'s state.
func (vrsb *Resource) ImportState(state io.Reader) error {
	vrsb.state = &vaultRabbitmqSecretBackendState{}
	if err := json.NewDecoder(state).Decode(vrsb.state); err != nil {
		return fmt.Errorf("decoding state into resource %s.%s: %w", vrsb.Type(), vrsb.LocalName(), err)
	}
	return nil
}

// State returns the state and a bool indicating if [Resource] has state.
func (vrsb *Resource) State() (*vaultRabbitmqSecretBackendState, bool) {
	return vrsb.state, vrsb.state != nil
}

// StateMust returns the state for [Resource]. Panics if the state is nil.
func (vrsb *Resource) StateMust() *vaultRabbitmqSecretBackendState {
	if vrsb.state == nil {
		panic(fmt.Sprintf("state is nil for resource %s.%s", vrsb.Type(), vrsb.LocalName()))
	}
	return vrsb.state
}

// Args contains the configurations for vault_rabbitmq_secret_backend.
type Args struct {
	// ConnectionUri: string, required
	ConnectionUri terra.StringValue `hcl:"connection_uri,attr" validate:"required"`
	// DefaultLeaseTtlSeconds: number, optional
	DefaultLeaseTtlSeconds terra.NumberValue `hcl:"default_lease_ttl_seconds,attr"`
	// Description: string, optional
	Description terra.StringValue `hcl:"description,attr"`
	// DisableRemount: bool, optional
	DisableRemount terra.BoolValue `hcl:"disable_remount,attr"`
	// Id: string, optional
	Id terra.StringValue `hcl:"id,attr"`
	// MaxLeaseTtlSeconds: number, optional
	MaxLeaseTtlSeconds terra.NumberValue `hcl:"max_lease_ttl_seconds,attr"`
	// Namespace: string, optional
	Namespace terra.StringValue `hcl:"namespace,attr"`
	// Password: string, required
	Password terra.StringValue `hcl:"password,attr" validate:"required"`
	// PasswordPolicy: string, optional
	PasswordPolicy terra.StringValue `hcl:"password_policy,attr"`
	// Path: string, optional
	Path terra.StringValue `hcl:"path,attr"`
	// Username: string, required
	Username terra.StringValue `hcl:"username,attr" validate:"required"`
	// UsernameTemplate: string, optional
	UsernameTemplate terra.StringValue `hcl:"username_template,attr"`
	// VerifyConnection: bool, optional
	VerifyConnection terra.BoolValue `hcl:"verify_connection,attr"`
}

type vaultRabbitmqSecretBackendAttributes struct {
	ref terra.Reference
}

// ConnectionUri returns a reference to field connection_uri of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) ConnectionUri() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("connection_uri"))
}

// DefaultLeaseTtlSeconds returns a reference to field default_lease_ttl_seconds of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) DefaultLeaseTtlSeconds() terra.NumberValue {
	return terra.ReferenceAsNumber(vrsb.ref.Append("default_lease_ttl_seconds"))
}

// Description returns a reference to field description of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Description() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("description"))
}

// DisableRemount returns a reference to field disable_remount of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) DisableRemount() terra.BoolValue {
	return terra.ReferenceAsBool(vrsb.ref.Append("disable_remount"))
}

// Id returns a reference to field id of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Id() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("id"))
}

// MaxLeaseTtlSeconds returns a reference to field max_lease_ttl_seconds of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) MaxLeaseTtlSeconds() terra.NumberValue {
	return terra.ReferenceAsNumber(vrsb.ref.Append("max_lease_ttl_seconds"))
}

// Namespace returns a reference to field namespace of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Namespace() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("namespace"))
}

// Password returns a reference to field password of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Password() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("password"))
}

// PasswordPolicy returns a reference to field password_policy of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) PasswordPolicy() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("password_policy"))
}

// Path returns a reference to field path of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Path() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("path"))
}

// Username returns a reference to field username of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) Username() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("username"))
}

// UsernameTemplate returns a reference to field username_template of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) UsernameTemplate() terra.StringValue {
	return terra.ReferenceAsString(vrsb.ref.Append("username_template"))
}

// VerifyConnection returns a reference to field verify_connection of vault_rabbitmq_secret_backend.
func (vrsb vaultRabbitmqSecretBackendAttributes) VerifyConnection() terra.BoolValue {
	return terra.ReferenceAsBool(vrsb.ref.Append("verify_connection"))
}

type vaultRabbitmqSecretBackendState struct {
	ConnectionUri          string  `json:"connection_uri"`
	DefaultLeaseTtlSeconds float64 `json:"default_lease_ttl_seconds"`
	Description            string  `json:"description"`
	DisableRemount         bool    `json:"disable_remount"`
	Id                     string  `json:"id"`
	MaxLeaseTtlSeconds     float64 `json:"max_lease_ttl_seconds"`
	Namespace              string  `json:"namespace"`
	Password               string  `json:"password"`
	PasswordPolicy         string  `json:"password_policy"`
	Path                   string  `json:"path"`
	Username               string  `json:"username"`
	UsernameTemplate       string  `json:"username_template"`
	VerifyConnection       bool    `json:"verify_connection"`
}
