// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package google_service_account_iam_policy

import (
	"encoding/json"
	"fmt"
	"github.com/golingon/lingon/pkg/terra"
	"io"
)

// New creates a new instance of [Resource].
func New(name string, args Args) *Resource {
	return &Resource{
		Args: args,
		Name: name,
	}
}

var _ terra.Resource = (*Resource)(nil)

// Resource represents the Terraform resource google_service_account_iam_policy.
type Resource struct {
	Name      string
	Args      Args
	state     *googleServiceAccountIamPolicyState
	DependsOn terra.Dependencies
	Lifecycle *terra.Lifecycle
}

// Type returns the Terraform object type for [Resource].
func (gsaip *Resource) Type() string {
	return "google_service_account_iam_policy"
}

// LocalName returns the local name for [Resource].
func (gsaip *Resource) LocalName() string {
	return gsaip.Name
}

// Configuration returns the configuration (args) for [Resource].
func (gsaip *Resource) Configuration() interface{} {
	return gsaip.Args
}

// DependOn is used for other resources to depend on [Resource].
func (gsaip *Resource) DependOn() terra.Reference {
	return terra.ReferenceResource(gsaip)
}

// Dependencies returns the list of resources [Resource] depends_on.
func (gsaip *Resource) Dependencies() terra.Dependencies {
	return gsaip.DependsOn
}

// LifecycleManagement returns the lifecycle block for [Resource].
func (gsaip *Resource) LifecycleManagement() *terra.Lifecycle {
	return gsaip.Lifecycle
}

// Attributes returns the attributes for [Resource].
func (gsaip *Resource) Attributes() googleServiceAccountIamPolicyAttributes {
	return googleServiceAccountIamPolicyAttributes{ref: terra.ReferenceResource(gsaip)}
}

// ImportState imports the given attribute values into [Resource]'s state.
func (gsaip *Resource) ImportState(state io.Reader) error {
	gsaip.state = &googleServiceAccountIamPolicyState{}
	if err := json.NewDecoder(state).Decode(gsaip.state); err != nil {
		return fmt.Errorf("decoding state into resource %s.%s: %w", gsaip.Type(), gsaip.LocalName(), err)
	}
	return nil
}

// State returns the state and a bool indicating if [Resource] has state.
func (gsaip *Resource) State() (*googleServiceAccountIamPolicyState, bool) {
	return gsaip.state, gsaip.state != nil
}

// StateMust returns the state for [Resource]. Panics if the state is nil.
func (gsaip *Resource) StateMust() *googleServiceAccountIamPolicyState {
	if gsaip.state == nil {
		panic(fmt.Sprintf("state is nil for resource %s.%s", gsaip.Type(), gsaip.LocalName()))
	}
	return gsaip.state
}

// Args contains the configurations for google_service_account_iam_policy.
type Args struct {
	// Id: string, optional
	Id terra.StringValue `hcl:"id,attr"`
	// PolicyData: string, required
	PolicyData terra.StringValue `hcl:"policy_data,attr" validate:"required"`
	// ServiceAccountId: string, required
	ServiceAccountId terra.StringValue `hcl:"service_account_id,attr" validate:"required"`
}

type googleServiceAccountIamPolicyAttributes struct {
	ref terra.Reference
}

// Etag returns a reference to field etag of google_service_account_iam_policy.
func (gsaip googleServiceAccountIamPolicyAttributes) Etag() terra.StringValue {
	return terra.ReferenceAsString(gsaip.ref.Append("etag"))
}

// Id returns a reference to field id of google_service_account_iam_policy.
func (gsaip googleServiceAccountIamPolicyAttributes) Id() terra.StringValue {
	return terra.ReferenceAsString(gsaip.ref.Append("id"))
}

// PolicyData returns a reference to field policy_data of google_service_account_iam_policy.
func (gsaip googleServiceAccountIamPolicyAttributes) PolicyData() terra.StringValue {
	return terra.ReferenceAsString(gsaip.ref.Append("policy_data"))
}

// ServiceAccountId returns a reference to field service_account_id of google_service_account_iam_policy.
func (gsaip googleServiceAccountIamPolicyAttributes) ServiceAccountId() terra.StringValue {
	return terra.ReferenceAsString(gsaip.ref.Append("service_account_id"))
}

type googleServiceAccountIamPolicyState struct {
	Etag             string `json:"etag"`
	Id               string `json:"id"`
	PolicyData       string `json:"policy_data"`
	ServiceAccountId string `json:"service_account_id"`
}
