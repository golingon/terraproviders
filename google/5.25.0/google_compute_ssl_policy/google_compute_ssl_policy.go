// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package google_compute_ssl_policy

import (
	"encoding/json"
	"fmt"
	"github.com/golingon/lingon/pkg/terra"
	"io"
)

// New creates a new instance of [Resource].
func New(name string, args Args) *Resource {
	return &Resource{
		Args: args,
		Name: name,
	}
}

var _ terra.Resource = (*Resource)(nil)

// Resource represents the Terraform resource google_compute_ssl_policy.
type Resource struct {
	Name      string
	Args      Args
	state     *googleComputeSslPolicyState
	DependsOn terra.Dependencies
	Lifecycle *terra.Lifecycle
}

// Type returns the Terraform object type for [Resource].
func (gcsp *Resource) Type() string {
	return "google_compute_ssl_policy"
}

// LocalName returns the local name for [Resource].
func (gcsp *Resource) LocalName() string {
	return gcsp.Name
}

// Configuration returns the configuration (args) for [Resource].
func (gcsp *Resource) Configuration() interface{} {
	return gcsp.Args
}

// DependOn is used for other resources to depend on [Resource].
func (gcsp *Resource) DependOn() terra.Reference {
	return terra.ReferenceResource(gcsp)
}

// Dependencies returns the list of resources [Resource] depends_on.
func (gcsp *Resource) Dependencies() terra.Dependencies {
	return gcsp.DependsOn
}

// LifecycleManagement returns the lifecycle block for [Resource].
func (gcsp *Resource) LifecycleManagement() *terra.Lifecycle {
	return gcsp.Lifecycle
}

// Attributes returns the attributes for [Resource].
func (gcsp *Resource) Attributes() googleComputeSslPolicyAttributes {
	return googleComputeSslPolicyAttributes{ref: terra.ReferenceResource(gcsp)}
}

// ImportState imports the given attribute values into [Resource]'s state.
func (gcsp *Resource) ImportState(state io.Reader) error {
	gcsp.state = &googleComputeSslPolicyState{}
	if err := json.NewDecoder(state).Decode(gcsp.state); err != nil {
		return fmt.Errorf("decoding state into resource %s.%s: %w", gcsp.Type(), gcsp.LocalName(), err)
	}
	return nil
}

// State returns the state and a bool indicating if [Resource] has state.
func (gcsp *Resource) State() (*googleComputeSslPolicyState, bool) {
	return gcsp.state, gcsp.state != nil
}

// StateMust returns the state for [Resource]. Panics if the state is nil.
func (gcsp *Resource) StateMust() *googleComputeSslPolicyState {
	if gcsp.state == nil {
		panic(fmt.Sprintf("state is nil for resource %s.%s", gcsp.Type(), gcsp.LocalName()))
	}
	return gcsp.state
}

// Args contains the configurations for google_compute_ssl_policy.
type Args struct {
	// CustomFeatures: set of string, optional
	CustomFeatures terra.SetValue[terra.StringValue] `hcl:"custom_features,attr"`
	// Description: string, optional
	Description terra.StringValue `hcl:"description,attr"`
	// Id: string, optional
	Id terra.StringValue `hcl:"id,attr"`
	// MinTlsVersion: string, optional
	MinTlsVersion terra.StringValue `hcl:"min_tls_version,attr"`
	// Name: string, required
	Name terra.StringValue `hcl:"name,attr" validate:"required"`
	// Profile: string, optional
	Profile terra.StringValue `hcl:"profile,attr"`
	// Project: string, optional
	Project terra.StringValue `hcl:"project,attr"`
	// Timeouts: optional
	Timeouts *Timeouts `hcl:"timeouts,block"`
}

type googleComputeSslPolicyAttributes struct {
	ref terra.Reference
}

// CreationTimestamp returns a reference to field creation_timestamp of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) CreationTimestamp() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("creation_timestamp"))
}

// CustomFeatures returns a reference to field custom_features of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) CustomFeatures() terra.SetValue[terra.StringValue] {
	return terra.ReferenceAsSet[terra.StringValue](gcsp.ref.Append("custom_features"))
}

// Description returns a reference to field description of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Description() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("description"))
}

// EnabledFeatures returns a reference to field enabled_features of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) EnabledFeatures() terra.SetValue[terra.StringValue] {
	return terra.ReferenceAsSet[terra.StringValue](gcsp.ref.Append("enabled_features"))
}

// Fingerprint returns a reference to field fingerprint of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Fingerprint() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("fingerprint"))
}

// Id returns a reference to field id of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Id() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("id"))
}

// MinTlsVersion returns a reference to field min_tls_version of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) MinTlsVersion() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("min_tls_version"))
}

// Name returns a reference to field name of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Name() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("name"))
}

// Profile returns a reference to field profile of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Profile() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("profile"))
}

// Project returns a reference to field project of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) Project() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("project"))
}

// SelfLink returns a reference to field self_link of google_compute_ssl_policy.
func (gcsp googleComputeSslPolicyAttributes) SelfLink() terra.StringValue {
	return terra.ReferenceAsString(gcsp.ref.Append("self_link"))
}

func (gcsp googleComputeSslPolicyAttributes) Timeouts() TimeoutsAttributes {
	return terra.ReferenceAsSingle[TimeoutsAttributes](gcsp.ref.Append("timeouts"))
}

type googleComputeSslPolicyState struct {
	CreationTimestamp string         `json:"creation_timestamp"`
	CustomFeatures    []string       `json:"custom_features"`
	Description       string         `json:"description"`
	EnabledFeatures   []string       `json:"enabled_features"`
	Fingerprint       string         `json:"fingerprint"`
	Id                string         `json:"id"`
	MinTlsVersion     string         `json:"min_tls_version"`
	Name              string         `json:"name"`
	Profile           string         `json:"profile"`
	Project           string         `json:"project"`
	SelfLink          string         `json:"self_link"`
	Timeouts          *TimeoutsState `json:"timeouts"`
}
