// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package google_compute_region_network_endpoint_group

import (
	terra "github.com/golingon/lingon/pkg/terra"
	hclwrite "github.com/hashicorp/hcl/v2/hclwrite"
)

type DataAppEngineAttributes struct {
	ref terra.Reference
}

func (ae DataAppEngineAttributes) InternalRef() (terra.Reference, error) {
	return ae.ref, nil
}

func (ae DataAppEngineAttributes) InternalWithRef(ref terra.Reference) DataAppEngineAttributes {
	return DataAppEngineAttributes{ref: ref}
}

func (ae DataAppEngineAttributes) InternalTokens() (hclwrite.Tokens, error) {
	return ae.ref.InternalTokens()
}

func (ae DataAppEngineAttributes) Service() terra.StringValue {
	return terra.ReferenceAsString(ae.ref.Append("service"))
}

func (ae DataAppEngineAttributes) UrlMask() terra.StringValue {
	return terra.ReferenceAsString(ae.ref.Append("url_mask"))
}

func (ae DataAppEngineAttributes) Version() terra.StringValue {
	return terra.ReferenceAsString(ae.ref.Append("version"))
}

type DataCloudFunctionAttributes struct {
	ref terra.Reference
}

func (cf DataCloudFunctionAttributes) InternalRef() (terra.Reference, error) {
	return cf.ref, nil
}

func (cf DataCloudFunctionAttributes) InternalWithRef(ref terra.Reference) DataCloudFunctionAttributes {
	return DataCloudFunctionAttributes{ref: ref}
}

func (cf DataCloudFunctionAttributes) InternalTokens() (hclwrite.Tokens, error) {
	return cf.ref.InternalTokens()
}

func (cf DataCloudFunctionAttributes) Function() terra.StringValue {
	return terra.ReferenceAsString(cf.ref.Append("function"))
}

func (cf DataCloudFunctionAttributes) UrlMask() terra.StringValue {
	return terra.ReferenceAsString(cf.ref.Append("url_mask"))
}

type DataCloudRunAttributes struct {
	ref terra.Reference
}

func (cr DataCloudRunAttributes) InternalRef() (terra.Reference, error) {
	return cr.ref, nil
}

func (cr DataCloudRunAttributes) InternalWithRef(ref terra.Reference) DataCloudRunAttributes {
	return DataCloudRunAttributes{ref: ref}
}

func (cr DataCloudRunAttributes) InternalTokens() (hclwrite.Tokens, error) {
	return cr.ref.InternalTokens()
}

func (cr DataCloudRunAttributes) Service() terra.StringValue {
	return terra.ReferenceAsString(cr.ref.Append("service"))
}

func (cr DataCloudRunAttributes) Tag() terra.StringValue {
	return terra.ReferenceAsString(cr.ref.Append("tag"))
}

func (cr DataCloudRunAttributes) UrlMask() terra.StringValue {
	return terra.ReferenceAsString(cr.ref.Append("url_mask"))
}

type DataServerlessDeploymentAttributes struct {
	ref terra.Reference
}

func (sd DataServerlessDeploymentAttributes) InternalRef() (terra.Reference, error) {
	return sd.ref, nil
}

func (sd DataServerlessDeploymentAttributes) InternalWithRef(ref terra.Reference) DataServerlessDeploymentAttributes {
	return DataServerlessDeploymentAttributes{ref: ref}
}

func (sd DataServerlessDeploymentAttributes) InternalTokens() (hclwrite.Tokens, error) {
	return sd.ref.InternalTokens()
}

func (sd DataServerlessDeploymentAttributes) Platform() terra.StringValue {
	return terra.ReferenceAsString(sd.ref.Append("platform"))
}

func (sd DataServerlessDeploymentAttributes) Resource() terra.StringValue {
	return terra.ReferenceAsString(sd.ref.Append("resource"))
}

func (sd DataServerlessDeploymentAttributes) UrlMask() terra.StringValue {
	return terra.ReferenceAsString(sd.ref.Append("url_mask"))
}

func (sd DataServerlessDeploymentAttributes) Version() terra.StringValue {
	return terra.ReferenceAsString(sd.ref.Append("version"))
}

type DataAppEngineState struct {
	Service string `json:"service"`
	UrlMask string `json:"url_mask"`
	Version string `json:"version"`
}

type DataCloudFunctionState struct {
	Function string `json:"function"`
	UrlMask  string `json:"url_mask"`
}

type DataCloudRunState struct {
	Service string `json:"service"`
	Tag     string `json:"tag"`
	UrlMask string `json:"url_mask"`
}

type DataServerlessDeploymentState struct {
	Platform string `json:"platform"`
	Resource string `json:"resource"`
	UrlMask  string `json:"url_mask"`
	Version  string `json:"version"`
}
