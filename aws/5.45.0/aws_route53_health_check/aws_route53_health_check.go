// CODE GENERATED BY github.com/golingon/lingon. DO NOT EDIT.

package aws_route53_health_check

import (
	"encoding/json"
	"fmt"
	"github.com/golingon/lingon/pkg/terra"
	"io"
)

// New creates a new instance of [Resource].
func New(name string, args Args) *Resource {
	return &Resource{
		Args: args,
		Name: name,
	}
}

var _ terra.Resource = (*Resource)(nil)

// Resource represents the Terraform resource aws_route53_health_check.
type Resource struct {
	Name      string
	Args      Args
	state     *awsRoute53HealthCheckState
	DependsOn terra.Dependencies
	Lifecycle *terra.Lifecycle
}

// Type returns the Terraform object type for [Resource].
func (arhc *Resource) Type() string {
	return "aws_route53_health_check"
}

// LocalName returns the local name for [Resource].
func (arhc *Resource) LocalName() string {
	return arhc.Name
}

// Configuration returns the configuration (args) for [Resource].
func (arhc *Resource) Configuration() interface{} {
	return arhc.Args
}

// DependOn is used for other resources to depend on [Resource].
func (arhc *Resource) DependOn() terra.Reference {
	return terra.ReferenceResource(arhc)
}

// Dependencies returns the list of resources [Resource] depends_on.
func (arhc *Resource) Dependencies() terra.Dependencies {
	return arhc.DependsOn
}

// LifecycleManagement returns the lifecycle block for [Resource].
func (arhc *Resource) LifecycleManagement() *terra.Lifecycle {
	return arhc.Lifecycle
}

// Attributes returns the attributes for [Resource].
func (arhc *Resource) Attributes() awsRoute53HealthCheckAttributes {
	return awsRoute53HealthCheckAttributes{ref: terra.ReferenceResource(arhc)}
}

// ImportState imports the given attribute values into [Resource]'s state.
func (arhc *Resource) ImportState(state io.Reader) error {
	arhc.state = &awsRoute53HealthCheckState{}
	if err := json.NewDecoder(state).Decode(arhc.state); err != nil {
		return fmt.Errorf("decoding state into resource %s.%s: %w", arhc.Type(), arhc.LocalName(), err)
	}
	return nil
}

// State returns the state and a bool indicating if [Resource] has state.
func (arhc *Resource) State() (*awsRoute53HealthCheckState, bool) {
	return arhc.state, arhc.state != nil
}

// StateMust returns the state for [Resource]. Panics if the state is nil.
func (arhc *Resource) StateMust() *awsRoute53HealthCheckState {
	if arhc.state == nil {
		panic(fmt.Sprintf("state is nil for resource %s.%s", arhc.Type(), arhc.LocalName()))
	}
	return arhc.state
}

// Args contains the configurations for aws_route53_health_check.
type Args struct {
	// ChildHealthThreshold: number, optional
	ChildHealthThreshold terra.NumberValue `hcl:"child_health_threshold,attr"`
	// ChildHealthchecks: set of string, optional
	ChildHealthchecks terra.SetValue[terra.StringValue] `hcl:"child_healthchecks,attr"`
	// CloudwatchAlarmName: string, optional
	CloudwatchAlarmName terra.StringValue `hcl:"cloudwatch_alarm_name,attr"`
	// CloudwatchAlarmRegion: string, optional
	CloudwatchAlarmRegion terra.StringValue `hcl:"cloudwatch_alarm_region,attr"`
	// Disabled: bool, optional
	Disabled terra.BoolValue `hcl:"disabled,attr"`
	// EnableSni: bool, optional
	EnableSni terra.BoolValue `hcl:"enable_sni,attr"`
	// FailureThreshold: number, optional
	FailureThreshold terra.NumberValue `hcl:"failure_threshold,attr"`
	// Fqdn: string, optional
	Fqdn terra.StringValue `hcl:"fqdn,attr"`
	// Id: string, optional
	Id terra.StringValue `hcl:"id,attr"`
	// InsufficientDataHealthStatus: string, optional
	InsufficientDataHealthStatus terra.StringValue `hcl:"insufficient_data_health_status,attr"`
	// InvertHealthcheck: bool, optional
	InvertHealthcheck terra.BoolValue `hcl:"invert_healthcheck,attr"`
	// IpAddress: string, optional
	IpAddress terra.StringValue `hcl:"ip_address,attr"`
	// MeasureLatency: bool, optional
	MeasureLatency terra.BoolValue `hcl:"measure_latency,attr"`
	// Port: number, optional
	Port terra.NumberValue `hcl:"port,attr"`
	// ReferenceName: string, optional
	ReferenceName terra.StringValue `hcl:"reference_name,attr"`
	// Regions: set of string, optional
	Regions terra.SetValue[terra.StringValue] `hcl:"regions,attr"`
	// RequestInterval: number, optional
	RequestInterval terra.NumberValue `hcl:"request_interval,attr"`
	// ResourcePath: string, optional
	ResourcePath terra.StringValue `hcl:"resource_path,attr"`
	// RoutingControlArn: string, optional
	RoutingControlArn terra.StringValue `hcl:"routing_control_arn,attr"`
	// SearchString: string, optional
	SearchString terra.StringValue `hcl:"search_string,attr"`
	// Tags: map of string, optional
	Tags terra.MapValue[terra.StringValue] `hcl:"tags,attr"`
	// TagsAll: map of string, optional
	TagsAll terra.MapValue[terra.StringValue] `hcl:"tags_all,attr"`
	// Type: string, required
	Type terra.StringValue `hcl:"type,attr" validate:"required"`
}

type awsRoute53HealthCheckAttributes struct {
	ref terra.Reference
}

// Arn returns a reference to field arn of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Arn() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("arn"))
}

// ChildHealthThreshold returns a reference to field child_health_threshold of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) ChildHealthThreshold() terra.NumberValue {
	return terra.ReferenceAsNumber(arhc.ref.Append("child_health_threshold"))
}

// ChildHealthchecks returns a reference to field child_healthchecks of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) ChildHealthchecks() terra.SetValue[terra.StringValue] {
	return terra.ReferenceAsSet[terra.StringValue](arhc.ref.Append("child_healthchecks"))
}

// CloudwatchAlarmName returns a reference to field cloudwatch_alarm_name of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) CloudwatchAlarmName() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("cloudwatch_alarm_name"))
}

// CloudwatchAlarmRegion returns a reference to field cloudwatch_alarm_region of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) CloudwatchAlarmRegion() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("cloudwatch_alarm_region"))
}

// Disabled returns a reference to field disabled of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Disabled() terra.BoolValue {
	return terra.ReferenceAsBool(arhc.ref.Append("disabled"))
}

// EnableSni returns a reference to field enable_sni of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) EnableSni() terra.BoolValue {
	return terra.ReferenceAsBool(arhc.ref.Append("enable_sni"))
}

// FailureThreshold returns a reference to field failure_threshold of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) FailureThreshold() terra.NumberValue {
	return terra.ReferenceAsNumber(arhc.ref.Append("failure_threshold"))
}

// Fqdn returns a reference to field fqdn of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Fqdn() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("fqdn"))
}

// Id returns a reference to field id of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Id() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("id"))
}

// InsufficientDataHealthStatus returns a reference to field insufficient_data_health_status of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) InsufficientDataHealthStatus() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("insufficient_data_health_status"))
}

// InvertHealthcheck returns a reference to field invert_healthcheck of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) InvertHealthcheck() terra.BoolValue {
	return terra.ReferenceAsBool(arhc.ref.Append("invert_healthcheck"))
}

// IpAddress returns a reference to field ip_address of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) IpAddress() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("ip_address"))
}

// MeasureLatency returns a reference to field measure_latency of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) MeasureLatency() terra.BoolValue {
	return terra.ReferenceAsBool(arhc.ref.Append("measure_latency"))
}

// Port returns a reference to field port of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Port() terra.NumberValue {
	return terra.ReferenceAsNumber(arhc.ref.Append("port"))
}

// ReferenceName returns a reference to field reference_name of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) ReferenceName() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("reference_name"))
}

// Regions returns a reference to field regions of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Regions() terra.SetValue[terra.StringValue] {
	return terra.ReferenceAsSet[terra.StringValue](arhc.ref.Append("regions"))
}

// RequestInterval returns a reference to field request_interval of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) RequestInterval() terra.NumberValue {
	return terra.ReferenceAsNumber(arhc.ref.Append("request_interval"))
}

// ResourcePath returns a reference to field resource_path of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) ResourcePath() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("resource_path"))
}

// RoutingControlArn returns a reference to field routing_control_arn of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) RoutingControlArn() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("routing_control_arn"))
}

// SearchString returns a reference to field search_string of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) SearchString() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("search_string"))
}

// Tags returns a reference to field tags of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Tags() terra.MapValue[terra.StringValue] {
	return terra.ReferenceAsMap[terra.StringValue](arhc.ref.Append("tags"))
}

// TagsAll returns a reference to field tags_all of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) TagsAll() terra.MapValue[terra.StringValue] {
	return terra.ReferenceAsMap[terra.StringValue](arhc.ref.Append("tags_all"))
}

// Type returns a reference to field type of aws_route53_health_check.
func (arhc awsRoute53HealthCheckAttributes) Type() terra.StringValue {
	return terra.ReferenceAsString(arhc.ref.Append("type"))
}

type awsRoute53HealthCheckState struct {
	Arn                          string            `json:"arn"`
	ChildHealthThreshold         float64           `json:"child_health_threshold"`
	ChildHealthchecks            []string          `json:"child_healthchecks"`
	CloudwatchAlarmName          string            `json:"cloudwatch_alarm_name"`
	CloudwatchAlarmRegion        string            `json:"cloudwatch_alarm_region"`
	Disabled                     bool              `json:"disabled"`
	EnableSni                    bool              `json:"enable_sni"`
	FailureThreshold             float64           `json:"failure_threshold"`
	Fqdn                         string            `json:"fqdn"`
	Id                           string            `json:"id"`
	InsufficientDataHealthStatus string            `json:"insufficient_data_health_status"`
	InvertHealthcheck            bool              `json:"invert_healthcheck"`
	IpAddress                    string            `json:"ip_address"`
	MeasureLatency               bool              `json:"measure_latency"`
	Port                         float64           `json:"port"`
	ReferenceName                string            `json:"reference_name"`
	Regions                      []string          `json:"regions"`
	RequestInterval              float64           `json:"request_interval"`
	ResourcePath                 string            `json:"resource_path"`
	RoutingControlArn            string            `json:"routing_control_arn"`
	SearchString                 string            `json:"search_string"`
	Tags                         map[string]string `json:"tags"`
	TagsAll                      map[string]string `json:"tags_all"`
	Type                         string            `json:"type"`
}
